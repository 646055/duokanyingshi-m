...
import { getComicUrl, getVipList, getVipPrice, dataMuch } from '../../api/index'
import { Notify } from 'vant'
export default {
  name: 'varVideoContainer',
  data() {
    return {
      user: {
        username: '',
  ...
  methods: {
    ...
    async getUrl() {
      ...
      if (this.$store.state.username === '') {
        ...
        this.user.comicName = this.$store.state.name
        // 发送请求
        console.log(this.user.username)
        const { data: res } = await getComicUrl(this.user)
